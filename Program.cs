    ...
    try
    {
        foreach (var p in purchases)
        {
            ...
            await db.ExecuteAsync(@"
                INSERT INTO purchase_logs (transaction_id, date, sku, qty, unit_cost, supplier)
                VALUES (@TransactionId, @Date, @SKU, @Qty, @UnitCost, @Supplier)", p, transaction);

            // 2. Create a new inventory lot for FIFO tracking
            await db.ExecuteAsync(@"
                INSERT INTO inventory_lots (sku, date_received, original_qty, remaining_qty, unit_cost)
